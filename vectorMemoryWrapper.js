...
class VectorMemoryWrapper {
    constructor(executablePath = './vector_memory') {
    ...
    _startProcess() {
        ...
        this.childProcess.stderr.on('data', (data) => {
            console.error(`VectorMemory C Error: ${data.toString()}`);
        });

        this.childProcess.on('close', (code) => {
            console.log(`VectorMemory C process exited with code ${code}`);
            this.childProcess = null;
        ...
        this.childProcess.on('error', (err) => {
            console.error(`Failed to start VectorMemory C process: ${err}`);
            this.childProcess = null;
            // Reject any pending promises
        ...
        return new Promise(resolve => {
            this.childProcess.stdout.once('data', () => resolve()); // Simple way to know it's ready
        });
    }
    ...
    async init(maxEntries, embeddingDim) {
        if (!this.initPromise) {
...
module.exports = VectorMemoryWrapper;

// Exemplo de uso (para testes internos do módulo Node.js)
async function testVectorMemoryWrapper() {
    console.log('Starting VectorMemoryWrapper test...');
    const vm = new VectorMemoryWrapper();

    try {
        // Initialize
        let response = await vm.init(10, 3);
        console.log('Init Response:', response);

        // Add entries
        response = await vm.addEntry('doc1', [0.1, 0.2, 0.3], 'Este é o documento um.');
        console.log('Add Doc1 Response:', response);
        response = await vm.addEntry('doc2', [0.9, 0.8, 0.7], 'Este é o documento dois.');
        console.log('Add Doc2 Response:', response);
        response = await vm.addEntry('doc3', [0.15, 0.25, 0.35], 'Este é o documento três.');
        console.log('Add Doc3 Response:', response);

        // Search
        response = await vm.search([0.1, 0.2, 0.3], 2);
        console.log('Search Response:', response);

        // Cleanup
        response = await vm.cleanup();
